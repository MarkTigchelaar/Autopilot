...
from SemanticAnalysis.type_resolved_module_collection import TypeResolvedModuleCollection


def run_type_setting_tests(progress_tracker: ProgressTracker) -> None:
    module_dependency_tests = open_json(
        "Testing/SemanticAnalysis/GlobalAnalysis/TypeResolution/tests.json"
    )
    ...
    for test in module_dependency_tests:
        # if test["main_module_path"] != "Case11":
        #     continue
        print(test["main_module_path"])
...
def run_test(test, test_case_folder, progress_tracker):
    ...
    type_resolver = TypeResolvedModuleCollection(parsed_modules)
    type_resolver.build_resolved_ast()
    if test["errors"] is None:
        test["errors"] = []
    detected_type_errors = error_manager.get_errors()
    #print(detected_errors)
    assert len(detected_type_errors) == len(test["errors"])


    for i, err in enumerate(detected_type_errors):
    ...
    #     assert parsed_modules.all_items_have_types()
