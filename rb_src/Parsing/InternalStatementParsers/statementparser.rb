...
class StatementParser
    ...
    def parse(parser, component_test = false, in_if = false)
        ...
        while(!isEOF(peekTok) and (is_interal_statement_keyword(peekTok) or isValidIdentifier(peekTok)))
            ...
            stmt = nil
            puts("in while loop in statement parser, peek is #{peekTok.getText()}")
            ...
            stmts.append(stmt)
            peekTok = parser.peek()
            puts("peek tok in statement parser: #{peekTok.getText()}")
            if(component_test and parser.hasErrors())
                puts("has errors")
                break
            end
        end
        puts("outside of while loop in statement parser, peek: #{peekTok.getText()}")
    ...
    def parseIf(parser)
        #if_parser = IfParser.new(@expression_parser, @sub_stmt_parser)
        @ifparser.parse(parser)
        #if_parser.parse(parser)
    end

    def parseElif(parser)
        #elif_parser = ElifParser.new(IfParser.new(@expression_parser, @sub_stmt_parser))
        @elifparser.parse(parser)
        #elif_parser.parse(parser)
    end
    ...
    def parseUnless(parser)
        #unless_parser = UnlessParser.new(IfParser.new(@expression_parser, @sub_stmt_parser))
        @unlessparser.parse(parser)
        #unless_parser.parse(parser)
    end
