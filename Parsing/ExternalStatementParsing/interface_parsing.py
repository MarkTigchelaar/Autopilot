...
def parse_interface(driver):
    ...
    interface_stmt = InterfaceStatement()
...
def functions_step(driver, interface_stmt):
    ...
    while True:
        ...
        if is_eof_type(peek_token):
            driver.add_error(peek_token, EOF_REACHED)
            return None
        if peek_token.internal_type not in (symbols.FUN, symbols.ACYCLIC):
            break
        acyclic_token = None
        if peek_token.internal_type == symbols.ACYCLIC:
            ...
            driver.discard_token()
            peek_token = driver.peek_token()
        if peek_token.internal_type == symbols.FUN:
...
def enforce_interface(interface_token):
    if interface_token.internal_type != symbols.INTERFACE:
