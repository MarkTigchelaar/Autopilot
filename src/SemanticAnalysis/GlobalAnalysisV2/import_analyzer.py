...
class ImportAnalyzer:
    ...
    def analyze_individual_module(self, index, raw_module, raw_modules):
        for import_index, import_stmt in enumerate(raw_module.imports):
            imported_module_maybe = self.check_imported_module_exists(index, import_stmt, raw_module, raw_modules)
            ...
            self.check_for_name_collisions_from_other_imported_modules(import_index, import_stmt, raw_module, raw_modules)
    ...
    def check_for_duplicate_imports(self, raw_module, import_stmt, import_index):
        for j, other_import_stmt in enumerate(raw_module.imports):
            if j <= import_index:
                continue
            if other_import_stmt.get_imported_name_token().literal == import_stmt.get_imported_name_token().literal:
                self.add_error(
    ...
    def check_imported_module_exists(self, index, import_stmt, raw_module, raw_modules):
        if not self.check_module_defined(import_stmt, raw_modules):
            return None
        possible_modules = self.get_possible_modules(import_stmt, raw_module, raw_modules)
    ...
    def get_possible_modules(self, import_stmt, raw_module, raw_modules):
        ...
        for other_raw_module in raw_modules:
            if other_raw_module == raw_module:
                continue
            if import_stmt.get_imported_name_token().literal == other_raw_module.get_module_name_token().literal:
    ...
    def collect_matching_modules(self, raw_module, possible_modules, import_stmt):
        ...
        path_matcher = ModulePathMatcher(
            module_id, import_path_to_module, raw_module.directory_path, self.error_manager
        )
        ...
        if len(modules_that_match) < 1:
            ...
            modules_that_match[0]


    def check_module_defined(self, import_stmt, raw_modules):
        for other_raw_module in raw_modules:
            if import_stmt.get_imported_name_token().literal == other_raw_module.get_module_name_token().literal:
                return True
        ...
        self.add_error(
            import_stmt.get_imported_name_token(),
            ErrMsg.INVALID_IMPORTED_MODULE
        )
        return False
