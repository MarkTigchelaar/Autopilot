...
class ModuleSaver(Saver):
    ...
    def save_to_db(self, database):      
        ...
        object_id = None
        if module_table.is_module_defined(module_name) and module_table.is_same_module(module_name):
            object_id = module_table.get_module_id_by_name_and_path(module_name, directory_path)
        else:
        ...
        if type_name_table.is_type_name_defined(module_name, object_id):
            if type_name_table.get_category_by_name_and_id(module_name, object_id) != "module_name":
