...
class ModuleSaver(Saver):
    ...
    def save_to_db(self, database):
        ...
        object_id = None
        if module_table.is_module_defined(
            module_name.literal
        ) and module_table.is_same_module(module_name, directory_path):
            object_id = module_table.get_module_id_by_name_and_path(
                module_name.literal, directory_path
            )
        else:
        ...
        if type_name_table.is_name_defined_in_table(module_name.literal, object_id):
            ...
            if (
                "module_name"
                not in type_name_table.get_categories_by_name_and_module_id(
