...
def validate_results(err_manager, test_case, tracker):
    ...
    for expected_error in expected_errors:
        ...
        # Yes, this is a hack, I should not have to plant the correct file as the error source,
        # but I don't want to change too many things
        test_case["file"] = expected_error["file"]
        record_component_test(test_case, tracker, expected_error["file"], actual_error.token.file_name)
        record_component_test(test_case, tracker, expected_error["token_literal"], actual_error.token.literal)
        record_component_test(test_case, tracker, expected_error["line_number"], actual_error.token.line_number)
        record_component_test(test_case, tracker, expected_error["column"], actual_error.token.column_number)
        record_component_test(test_case, tracker, expected_error["message"], actual_error.message)
        if actual_error.shadowed_token:
            record_component_test(test_case, tracker, expected_error["shadowed_file"], actual_error.shadowed_token.file_name)
            record_component_test(test_case, tracker, expected_error["shadowed_token_literal"], actual_error.shadowed_token.literal)
            record_component_test(test_case, tracker, expected_error["shadowed_line_number"], actual_error.shadowed_token.line_number)
            record_component_test(test_case, tracker, expected_error["shadowed_column"], actual_error.shadowed_token.column_number)
        else:
            record_component_test(test_case, tracker, expected_error["shadowed_file"], None)
            record_component_test(test_case, tracker, expected_error["shadowed_token_literal"], None)
            record_component_test(test_case, tracker, expected_error["shadowed_line_number"], None)
            record_component_test(test_case, tracker, expected_error["shadowed_column"], None)
...
def check_for_token_and_parser_errors(err_manager, test_case, tracker):
    if len(err_manager.tokenizer_errors) > 0:
        raise Exception("INTERNAL ERROR: tokenizer error(s) found in semantic analysis test")
