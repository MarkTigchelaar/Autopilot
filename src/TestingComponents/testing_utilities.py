    ...
    for test in component_tests:
        ...
        if err_manager.has_errors() and test["errors"] is None:
            ...
            for err in test["errors"]:
                ...
                record_component_test(test, tracker, err["file"], parse_err.token.file_name)
                record_component_test(test, tracker, err["tokenLiteral"], parse_err.token.literal)
                record_component_test(test, tracker, err["lineNumber"], parse_err.token.line_number)
                ...
                if "column_number" in err and err["column_number"]:
                    record_component_test(test, tracker, err["column_number"], parse_err.token.column_number)
...
def tokenizer_test(tokenizer: Tokenizer, test: dict, tracker, current_dir: str, err_manager: ErrorManager) -> None:
    ...
    while tokenizer.has_tokens():
        ...
        if err_manager.has_errors():
            ...
            print("ERROR: tests expected no errors, but Tokenizer generated errors! See error file.")
    ...
    if type(expected_ast) is dict:
        for key in expected_ast:
            if key not in ast_json:
                record_component_test(test, tracker, ",".join(list(expected_ast.keys())), ",".join(list(ast_json.keys())))
                return
            else:
