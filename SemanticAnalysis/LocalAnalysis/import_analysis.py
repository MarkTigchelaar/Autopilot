...
def check_import_path(analyzer, import_node):
    ...
    for i, node in enumerate(import_path):
        if node.direction_token and node.direction_token.internal_type in (symbols.DOT, symbols.COLON):
            if i != 0:
                dot_or_colon_found = True
        if node.direction_token and node.direction_token.internal_type == symbols.RANGE:
