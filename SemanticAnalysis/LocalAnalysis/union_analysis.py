from ErrorHandling.semantic_error_messages import (
    DUPLICATE_UNION_FIELD_NAME,
    DUPLICATE_UNION_FIELD_TYPE,
    UNION_AND_FIELD_NAME_COLLISION,
    UNION_AND_FIELD_TYPE_COLLISION
)
...
def check_fields_for_duplicate_types(analyzer, field_one, field_two):
    if field_one.type_token.internal_type == field_two.type_token.internal_type:
        if field_one.type_token.internal_type != symbols.IDENTIFIER:
